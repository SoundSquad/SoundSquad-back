...
export const searchCategoryConcert = async( req : Request, res : Response )=>{
  try{
    ...
    return res.status(500).json({ 
      msg: 'concert 목록 검색 중 오류가 발생했습니다.',
      flag: false
    });
  }
}
...
export const getDetailConcert = async ( req : Request, res : Response )=>{
  try {
    ...
    if(!concert_num){
      logger.error(' getDetailConcert - 400 ' );
      return res.status(400).json({ 
        msg: '요청 대상이 잘못되었습니다.',
        flag: false 
      });
...
export const getDetailArtistReview = async (req: Request, res: Response) => {
  try {
    ...
    if (isNaN(artist_num) || artist_num <= 0) {
      logger.error(' getDetailArtistReview - 400 ');
      return res.status(400).json({ 
        msg: '유효한 아티스트 번호를 입력해주세요',
        flag: false 
      });
    ...
    return res.status(500).json({ 
      msg: '아티스트 공연 리뷰 목록 검색 중 오류가 발생했습니다.',
      flag: false 
    });
  }
};
...
export const getSearchCommunityList= async (req: Request, res: Response) => {
  try {
    ...
    if (isNaN(page) || page < 1) {
      logger.error(' getSearchCommunityList - 400 ');
      return res.status(400).json({ 
        msg: '유효한 페이지 번호를 입력해주세요',
        flag: false 
      });
    ...
    return res.status(500).json({ 
      msg: 'Community 게시글 목록 검색 중 오류가 발생했습니다.',
      flag: false 
    });
  }
};
...
export const getCategoryCommunityList = async (req: Request, res: Response) => {
  try {
    ...
    if (isNaN(page) || page < 1) {
      logger.error(' getCategoryCommunityList - 400 ');
      return res.status(400).json({ 
        msg: '유효한 페이지 번호를 입력해주세요',
        flag: false 
      });
